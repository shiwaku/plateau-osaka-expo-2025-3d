...
// 万博会場（夢洲）の中心座標
const EXPO_CENTER: [number, number] = [135.3900, 34.6500]
...
const findMinZ = (fc: FeatureCollection): number => {
  ...
  const check = (c: any): void => {
    if (typeof c[0] === 'number') { if (c.length >= 3) minZ = Math.min(minZ, c[2]); return }
    c.forEach(check)
  }
  fc.features.forEach(f => { if (f.geometry) check((f.geometry as any).coordinates) })
...
const shiftZ = (fc: FeatureCollection, offset: number): FeatureCollection => {
  ...
  return {
    ...
    features: fc.features.map(f => {
      ...
      return { ...f, geometry: { ...geom, coordinates: adjustCoords(geom.coordinates) } as Geometry }
    }),
  }
}
...
const style: StyleSpecification = {
  ...
  sources: {
    aerial: {
      type: 'raster',
      tiles: ['https://xs489works.xsrv.jp/pmtiles-data/plateau-osaka-expo-2025-3d/tiles/{z}/{x}/{y}.png'],
      tileSize: 256,
      minzoom: 9,
      maxzoom: 18,
    },
...
const setLoadingText = (text: string) => { loadingText.textContent = text }
...
const installationData = shiftZ(
  await fetch('https://xs489works.xsrv.jp/pmtiles-data/plateau-osaka-expo-2025-3d/geojson/BuildingInstallation.geojson').then(r => r.json()) as FeatureCollection,
  zOffset
)

setLoadingText('CityFurniture.geojson を読み込み中... (大容量)')
const cityFurnitureData = shiftZ(
  await fetch('https://xs489works.xsrv.jp/pmtiles-data/plateau-osaka-expo-2025-3d/geojson/CityFurniture.geojson').then(r => r.json()) as FeatureCollection,
  zOffset
)
...
map.addControl(new AttributionControl({
  customAttribution: '<a href="https://www.geospatial.jp/ckan/dataset/plateau-27999-osaka-shi-2025" target="_blank">© 2025年大阪・関西万博会場　3D都市モデル（Project PLATEAU）</a>',
  compact: false,
}))
...
const showPopup = (layerType: string, props: Record<string, unknown>) => {
  ...
  const rows = Object.entries(props)
    ...
    .map(([k, v]) => `<tr><td class="key">${k}</td><td class="val">${String(v)}</td></tr>`)
    .join('')
...
const buildLayers = () => [
  new GeoJsonLayer({
    ...
    pickable: true,
    onClick: (info) => {
      if (info.object?.properties) showPopup('installation', info.object.properties as Record<string, unknown>)
    },
  ...
  new GeoJsonLayer({
    ...
    pickable: true,
    onClick: (info) => {
      if (info.object?.properties) showPopup('building', info.object.properties as Record<string, unknown>)
    },
    onHover: (info) => { map.getCanvas().style.cursor = info.object ? 'pointer' : '' },
  }),
  new GeoJsonLayer({
    ...
    pickable: true,
    onClick: (info) => {
      if (info.object?.properties) showPopup('furniture', info.object.properties as Record<string, unknown>)
    },
    onHover: (info) => { map.getCanvas().style.cursor = info.object ? 'pointer' : '' },
  }),
]
...
document.getElementById('toggle-installation')!.addEventListener('change', (e) => {
  showInstallation = (e.target as HTMLInputElement).checked
  deckOverlay.setProps({ layers: buildLayers() })
})
document.getElementById('toggle-city-furniture')!.addEventListener('change', (e) => {
